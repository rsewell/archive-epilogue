...
set_env(char *key, char *value) {
    ...
    if (!SetEnvironmentVariableW(wkey, wvalue)) error("SetEnvironmentVariable(\"%s\", \"%s\") failed!", key, value);
    efree(wkey);
    efree(wvalue);
#else
    ...
    if (putenv(str) != 0) error("putenv(\"%s\") failed!", str);
...
static void add_boot_config(void) {
    int i;
    if (boot_script) add_args("-boot", boot_script, NULL);
...
* / #end
    ...
    while (i < argc) {
        if (argv[i][0] == '+') {
            if (argv[i][1] == 'M' && argv[i][2] == 'Y' && argv[i][3] == 'm') {
                if (argv[i][4] == '\0') {
                    if (++i < argc) malloc_lib = argv[i];
                    else usage("+MYm");
                } else malloc_lib = &argv[i][4];
            }
        } else if (argv[i][0] == '-') {
            if (strcmp(argv[i], "-smp") == 0) {
                if (i + 1 >= argc) goto smp;
    ...
    if (malloc_lib) {
        if (strcmp(malloc_lib, "libc") != 0) usage("+MYm");
    }
    ...
    while (i < argc) {
        if (!process_args) {    /* Copy arguments after '-extra' */
            ...
            case '-':
                ...
                case 'b':
                    if (strcmp(argv[i], "-boot") == 0) {
                        if (boot_script) error("Conflicting -boot options");
                        if (got_start_erl) error("Conflicting -start_erl and -boot options");
                        if (i + 1 >= argc) usage("-boot");
                ...
                case 'c':
                    if (strcmp(argv[i], "-compile") == 0) {
                        ...
                        if (got_start_erl) error("Conflicting -start_erl and -config options");
                        if (i + 1 >= argc) usage("-config");
                        config_script_cnt++;
                ...
                case 's': /* -sname NAME */
                    if (strcmp(argv[i], "-sname") == 0) {
                        ...
                        if (i + 1 < argc && argv[i + 1][0] != '-') {
                            ...
                            i++;
                        } else get_start_erl_data((char *)NULL);
                    }
#endif
else if (strcmp(argv[i], "-start_epmd") == 0) {
                        if (i + 1 >= argc) usage("-start_epmd");
                        ...
                        if (strcmp(argv[i + 1], "true") == 0) {
                            ...
                            no_epmd = 1;
                        } else usage_format("Expected boolean argument for \'-start_epmd\'.\n");
            ...
            case '+':
                ...
                case 'W':
                case 'K':
                    if (argv[i][2] != '\0') goto the_default;
                ...
                case 'I':
                    if (argv[i][2] == 'O' && (argv[i][3] == 't' || argv[i][3] == 'p')) {
                        if (argv[i][4] != '\0') goto the_default;
                        argv[i][0] = '-';
                        add_Eargs(argv[i]);
                        add_Eargs(argv[i + 1]);
                        i++;
                    ...
                    if (argv[i][2] == 'O' && argv[i][3] == 'P' &&
                        ...
                        if (argv[i][5] != '\0') goto the_default;
                        argv[i][0] = '-';
                        add_Eargs(argv[i]);
                        add_Eargs(argv[i + 1]);
                        i++;
                ...
                case 'S':
                    if (argv[i][2] == 'P') {
                        if (argv[i][3] != '\0') goto the_default;
                    } else if (argv[i][2] == 'D') {
                        char *type = argv[i] + 3;
                        if (strncmp(type, "cpu", 3) != 0 &&
                            strncmp(type, "Pcpu", 4) != 0 &&
                            strncmp(type, "io", 2) != 0) usage(argv[i]);
                        if ((argv[i][3] == 'c' && argv[i][6] != '\0') ||
                            (argv[i][3] == 'P' && argv[i][7] != '\0') ||
                            (argv[i][3] == 'i' && argv[i][5] != '\0')) goto the_default;
                    } else if (argv[i][2] != '\0') goto the_default;
                    if (i + 1 >= argc) usage(argv[i]);
                    argv[i][0] = '-';
                    add_Eargs(argv[i]);
                    add_Eargs(argv[i + 1]);
                ...
                case 'M':
                    {
                        int x;
                        for (x = 0; plusM_au_allocs[x]; x++) if (plusM_au_allocs[x] == argv[i][2]) break;
                        if ((plusM_au_allocs[x]
                            ...
                            if (i + 1 >= argc
                                || argv[i + 1][0] == '-'
                                || argv[i + 1][0] == '+') usage(argv[i]);
                            argv[i][0] = '-';
                            add_Eargs(argv[i]);
                            add_Eargs(argv[i + 1]);
                            i++;
                        } else goto the_default;
                        break;
                    }
                case 'h':
                ...
                case 'r':
                    if (!is_one_of_strings(&argv[i][2],
                                           plusr_val_switches)) goto the_default;
                    else {
                        if (i + 1 >= argc
                            || argv[i + 1][0] == '-'
                            || argv[i + 1][0] == '+') usage(argv[i]);
                ...
                case 's':
                    if (!is_one_of_strings(&argv[i][2],
                                           pluss_val_switches)) goto the_default;
                    else {
                        if (i + 1 >= argc
                            || argv[i + 1][0] == '-'
                            || argv[i + 1][0] == '+') usage(argv[i]);
                    ...
                    break;
                case 'p':
                    if (argv[i][2] != 'c' || argv[i][3] != '\0') goto the_default;
                    if (i + 1 >= argc) usage(argv[i]);
                    argv[i][0] = '-';
                    add_Eargs(argv[i]);
                    add_Eargs(argv[i + 1]);
...
#undef ADD_BOOT_CONFIG
    ...
    if (isdistributed && !no_epmd) start_epmd(epmd_prog);
    ...
    if (start_detached && get_env("ERL_CONSOLE_MODE")) {
        ...
        for (i = EargsCnt; i > 0; i--) Eargsp[i + 1] = Eargsp[i - 1]; /* Two args to insert */
    ...
    for (i = 0; i < argsCnt; i++) Eargsp[EargsCnt++] = argsp[i];
    Eargsp[EargsCnt] = NULL;

    if (print_qouted_cmd_exit) {
        printf("\"%s\" ", emu);
        for (i = 1; i < EargsCnt; i++) printf("\"%s\" ", Eargsp[i]);
        printf("\n");
        exit(0);
    }

    if (print_args_exit) {
        for (i = 1; i < EargsCnt; i++) printf("%s\n", Eargsp[i]);
        exit(0);
    }

    if (verbose) {
        printf("Executing: %s", emu);
        for (i = 0; i < EargsCnt; i++) printf(" %s", Eargsp[i]);
        printf("\n\n");
    }

#ifdef __WIN32__

    if (EargsSz != EargsCnt + 1) Eargsp = (char **)erealloc((void *)Eargsp, (EargsCnt + 1) *
                                                            sizeof(char *));
    efree((void *)argsp);
...
skip_arg_massage:
    if (start_detached) {
        ...
        if (reset_cerl_detached) putenv("CERL_DETACHED_PROG=");
...
add_arg(char *new_arg) {
    if (argsCnt >= argsSz) argsp = (char **)erealloc((void *)argsp,
...
ensure_EargsSz(int sz) {
    if (EargsSz < sz) Eargsp = (char **)erealloc((void *)Eargsp,
                                                 sizeof(char *) * (EargsSz = sz));
}

static void
add_Eargs(char *new_arg) {
    if (EargsCnt >= EargsSz) Eargsp = (char **)erealloc((void *)Eargsp,
...
erealloc(void *p, size_t size) {
    ...
    if (res == NULL) error("Insufficient memory");
    return res;
}
...
static void get_start_erl_data(char *file) {
    ...
    char *otpstring;
    char *tprogname;
    if (boot_script) error("Conflicting -start_erl and -boot options");
    if (config_scripts) error("Conflicting -start_erl and -config options");
    env = get_env("RELDIR");
    if (env) reldir = strsave(env);
    else {
        erts_snprintf(tmpbuffer, sizeof(tmpbuffer), "%s/releases", rootdir);
        reldir = strsave(tmpbuffer);
    }
    free_env_val(env);
    if (file == NULL) erts_snprintf(start_erl_data, sizeof(start_erl_data), "%s/start_erl.data", reldir);
    else erts_snprintf(start_erl_data, sizeof(start_erl_data), "%s", file);
    fp = _open(start_erl_data, _O_RDONLY);
    if (fp == -1) error("open failed on %s", start_erl_data);
    else {
        if ((bytesread = _read(fp, tmpbuffer, 512)) <= 0) error("Problem reading file %s", start_erl_data);
        else {
...
get_home(void) {
    ...
    if (!homedrive || !homepath) {
        ...
        } else error("HOMEDRIVE or HOMEPATH is not set and GetWindowsDir failed");
    } else {
...
get_home(void) {
    home = get_env("HOME");
    if (home == NULL) error("HOME must be set");
}
...
static char** build_args_from_string(char *string) {
    ...
    char *p = string;
    enum {
        Start, Build, Build0, BuildSQuoted, BuildDQuoted, AcceptNext
    } state;
...
#define SAVE_CHAR(C)						\
    ...
    if (astr[0] == '\0') res = NULL;
    else res = build_args_from_string(astr);
...
save_arg(argv_buf *abp, char *arg) {
    if (abp->size <= abp->argc) {
        if (!abp->argv) abp->argv = emalloc(sizeof(char *) * (abp->size = 100));
        else abp->argv = erealloc(abp->argv, sizeof(char *) * (abp->size += 100));
    }
    ...
    while (argv) {
        ...
        while (argv[i]) {
            if (strcmp(argv[i], "-args_file") == 0) {
                char **new_argv;
                char *fname;
                if (!argv[++i]) usage("-args_file");
                fname = argv[i++];
                new_argv = read_args_file(fname);
                if (new_argv) {
                    if (argv[i]) push_argv(&stck, argv, i);
                    else efree(argv);
                    i = 0;
                    argv = new_argv;
                }
                ...
                if (strcmp(argv[i], "-extra") == 0) {
                    i++;
                    while (argv[i]) save_arg(xabp, argv[i++]);
                    break;
                }
                save_arg(abp, argv[i++]);
            }
...
build_new_argv:
    ...
    while (avv[vix].argv) {
        ...
        while (ix < ac && av[ix]) {
            if (strcmp(av[ix], "-args_file") == 0) {
                if (++ix == ac) usage("-args_file");
                get_file_args(av[ix++], &ab, &xab);
            } else {
                if (strcmp(av[ix], "-extra") == 0) {
                    ix++;
                    while (ix < ac && av[ix]) save_arg(&xab, av[ix++]);
                    break;
                }
                save_arg(&ab, av[ix++]);
            }
    ...
    if (xab.argc) {
        save_arg(&ab, "-extra");
        for (ix = 0; ix < xab.argc; ix++) save_arg(&ab, xab.argv[ix]);
        efree(xab.argv);
    }
...
static WCHAR* latin1_to_utf16(char *str) {
    ...
    for (i = 0; i < len; ++i) wstr[i] = (WCHAR)str[i];
    wstr[len] = L'\0';
    return wstr;
}
